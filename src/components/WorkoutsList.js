...
const ClientProfile = ({ clients, setClients }) => {
  ...
  const handleDelete = async (clientId, workoutId) => {
    if (
      window.confirm("Are you sure you want to delete? You cannot recover it!")
    ) {
      try {
        ...
        const updatedClients = clients.map((client) =>
          client.clientId === parseInt(clientId)
            ? {
                ...client,
                workouts: client.workouts.filter(
  ...
  const displayWorkouts = sortedWorkouts.map((workout) => {
    return (
      <div key={workout.workoutId} className="card">
        ...
        <button
          onClick={() => handleDelete(client.clientId, workout.workoutId)}
...
return (
  ...
  {client ? (
    <>
      ...
      <Link to={`/clients/${clientId}/workout/new`}>
        <h2 className="text-center mx-auto btn btn-success w-100 my-2">
          Add Workout +
        </h2>
      </Link>
